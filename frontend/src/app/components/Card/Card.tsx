  ...
  return (
    <div className="card text-white bg-secondary v-card w-100 shadow-none p-4">
        ...
        <div className="names">
          ...
          <div className="text-info d-flex align-items-center pt-1">
            ...
            <Badge className="bg-primary d-flex align-items-center ml-2 rounded-lg px-2">
              <Icon Component={Thumb} size={[0.7, 0.7]} /> <span className="text-white pl-1">104</span>{" "}
            </Badge>
          </div>
        </div>
        <div className="people-count d-flex align-items-center">
          <Icon Component={Users} size={[1.2, 1.2]} />
          <span className="font-weight-bold ml-2">12</span>
        </div>
      </div>
      <div className="card-body p-0 mt-3 v-card-body">
        {room.tags.map((tag) => (
          <Badge className="bg-light text-white mr-2 rounded-lg px-2" key={tag.label}>
            {tag.label}
          </Badge>
        ))}
      </div>
      <div className="card-footer p-0 w-100 pt-3">
        <div className="d-flex justify-content-between align-items-center w-100">
          <div className="song d-flex">
            <Avatar src="https://picsum.photos/seed/picsum/200/300" alt="song-album-art" size="md" />
            <div className="song-details ml-3 ">
              <p className="font-weight-bold mb-0 song-name">This is the name of the song, and it can be long</p>
      ...
      <div className="join-room p-2 bg-secondary">
        <button className="btn btn-primary btn-sm font-weight-bold px-3" type="button">
          Join Room
        </button>
      </div>
    </div>
